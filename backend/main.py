...
# Fix 9: Use modern lifespan pattern instead of deprecated on_event
@asynccontextmanager
...
# Fix 10: Tighten CORS — allow only the frontend origin (overridable via env)
app.add_middleware(
    CORSMiddleware,
...
# Fix 8: asyncio lock to guard shared realtime_cache in async context
_realtime_cache_lock = asyncio.Lock()

# Fix 7: Module-level cache for /historical-accuracy (expensive endpoint)
...
async def predict_load(request: ForecastRequest):
    try:
        ...
        # Fix 11: Use >= 7 (not > 7) so a 7-day range is valid, matching frontend
...
async def whatif_predict(request: WhatIfRequest):
    ...
    try:
        ...
        all_b = [v for v in (b_lstm + b_gru) if v is not None]
        all_m = [v for v in (m_lstm + m_gru) if v is not None]
        ...
        return WhatIfResponse(
            timestamps=timestamps,
            baseline_lstm=b_lstm,
            baseline_gru=b_gru,
            modified_lstm=m_lstm,
            modified_gru=m_gru,
            delta_lstm=d_lstm,
            delta_gru=d_gru,
            summary=summary,
...
async def get_realtime_status():
    ...
    # Fix 8: Use asyncio lock to protect shared cache dict in async context
...
def get_historical_accuracy():
    ...
    # Fix 7: Return cached result if still valid (1-hour TTL)
    now = datetime.now()

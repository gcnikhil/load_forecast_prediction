...
const Dashboard = () => {
    ...
    const getModelStats = () => {
        if (!data?.loads_lightgbm_lstm?.length) return null;
        const lstmLoads = data.loads_lightgbm_lstm.filter(v => v !== null && !isNaN(v));
        const gruLoads  = data.loads_lightgbm_gru.filter(v => v !== null && !isNaN(v));
        if (!lstmLoads.length || !gruLoads.length) return null;

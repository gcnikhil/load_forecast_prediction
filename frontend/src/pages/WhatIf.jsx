...
/* ─── Heuristic Helpers for Features 2-6 ──────────────── */
...
const getImpactMetrics = (result) => {
    ...
    const baseLstm = result.baseline_lstm;
    const modLstm = result.modified_lstm;
    
    let baseMax = -Infinity, baseMaxIdx = -1;
    let modMax = -Infinity, modMaxIdx = -1;
    
    for (let i = 0; i < baseLstm.length; i++) {
        if (baseLstm[i] > baseMax) { baseMax = baseLstm[i]; baseMaxIdx = i; }
        if (modLstm[i] > modMax) { modMax = modLstm[i]; modMaxIdx = i; }
    }
    ...
    const deficitMWh = (result.delta_lstm.reduce((sum, d) => sum + Math.max(0, d), 0) / 4).toFixed(1);
...
export default function WhatIf() {
    ...
    const chartData = simResult
        ? simResult.timestamps.map((t, i) => ({
            ...
            'Baseline Load': simResult.baseline_lstm[i],
            'Baseline (Secondary)': simResult.baseline_gru[i],
            'Modified Load': simResult.modified_lstm[i],
            'Modified (Secondary)': simResult.modified_gru[i],
            delta: simResult.delta_lstm[i],
        }))
        : [];
    ...
    return (
        <div className="space-y-6">
            ...
            {dateError && (
                <p className="text-red-400 text-xs font-mono px-1">⚠ {dateError}</p>
            )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
                ...
                <div className="xl:col-span-2 border border-[#00FFF6]/20 bg-[#0B0F1A] flex flex-col" style={{ height: 580 }}>
                    ...
                    <div className="border-t border-[#00FFF6]/20 p-3 flex flex-col gap-3">
                        ...
                        {!apiKey && (
                            <p className="text-[10px] font-mono text-red-400/70">⚠ Enter your OpenRouter API key above to use the LLM.</p>
                        )}
                    </div>
                </div>
                ...
                <div className="xl:col-span-3 space-y-4">
                    ...
                    {simResult && (
                        <>
                            ...
                            <div className="border border-[#00FFF6]/10 bg-[#0B0F1A] p-4">
                                ...
                                <ResponsiveContainer width="100%" height={260}>
                                    <ComposedChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
                                        ...
                                        />
                                        <Line type="monotone" dataKey="Baseline Load" stroke="#7B61FF" strokeWidth={1} dot={false} strokeDasharray="4 2" />
                                        <Line type="monotone" dataKey="Baseline (Secondary)" stroke="#7B61FF66" strokeWidth={1} dot={false} strokeDasharray="4 2" />
                                        <Line type="monotone" dataKey="Modified Load" stroke="#00FFF6" strokeWidth={2} dot={false} />
                                        <Line type="monotone" dataKey="Modified (Secondary)" stroke="#00FFF6aa" strokeWidth={1.5} dot={false} />
                                    </ComposedChart>
                                </ResponsiveContainer>
                            </div>
                            ...
                            {/* Feature 2: Impact Analysis Metrics */}
                            {(() => {
                            ...
                            {/* Feature 4: Explainability Panel */}
                            {(() => {
                            ...
                            {(() => {
                                ...
                                for (let i = 0; i < simResult.timestamps.length; i++) {
                                    ...
                                    if (date.getHours() !== currentHour) {
                                        ...
                                        const val = simResult.modified_lstm[i];
                                        const pct = val / 16000;

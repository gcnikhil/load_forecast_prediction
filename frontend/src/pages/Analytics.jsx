...
const Analytics = () => {
    ...
    const varianceData = timestamps
        ...
        .map((ts, idx) => {
            const i = idx * 30;
            return { time: ts.split(' ')[1]?.substring(0, 5) || ts, diff: Math.round(Math.abs(lstmLoads[i] - gruLoads[i])) };
        });

    const pieData = [
        { name: 'LSTM', value: Math.round(lstmLoads.reduce((a, b) => a + b, 0) / 1000) },
        { name: 'GRU', value: Math.round(gruLoads.reduce((a, b) => a + b, 0) / 1000) }
    ];

    const COLORS = ['#00FFF6', '#FF2A6D'];
    const avgDiff = lstmLoads.reduce((acc, val, i) => acc + Math.abs(val - gruLoads[i]), 0) / lstmLoads.length;
    const deviationPct = ((avgDiff / lstmAvg) * 100).toFixed(2);
        ...
        <div className="space-y-5 font-mono w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-[#0B0F1A] border border-[#1e293b] p-4 rounded-lg">
                    ...
                    <div style={{ width: '100%', height: 240 }}>
                        <ResponsiveContainer>
                            <LineChart data={varianceData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                ...
                <div className="bg-[#0B0F1A] border border-[#1e293b] p-4 rounded-lg">
                    <h3 className="text-sm font-bold text-slate-400 mb-2 flex items-center gap-2">
                        <Target className="w-4 h-4 text-green-400" /> LOAD SHARE
                    </h3>
                    <div style={{ width: '100%', height: 240 }}>
                        <ResponsiveContainer>
                            <PieChart>
                                ...
                                <Tooltip contentStyle={{ backgroundColor: '#0B0F1A', border: '1px solid #00FFF6', borderRadius: 4, fontSize: 11 }} formatter={(v) => [`${v.toLocaleString()} GWh`]} />
                                <Legend wrapperStyle={{ fontSize: 11 }} />
                            </PieChart>
                        </ResponsiveContainer>
                    </div>

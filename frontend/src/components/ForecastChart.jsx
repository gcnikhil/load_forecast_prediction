import {
    ...
    Brush, Legend, ReferenceLine, ReferenceDot, ComposedChart, Line
} from 'recharts';

// Fix 13: Single stable CustomTooltip defined OUTSIDE the component so it
// is not re-created on every render (prevents tooltip flickering).
...
const ForecastChart = ({ data }) => {
    ...
    // Fix 14: Find peak and minimum indices for ReferenceDot annotations
    const peakIdx = chartData.reduce((iMax, d, i) => d.lstm_hybrid > chartData[iMax].lstm_hybrid ? i : iMax, 0);
    const minIdx  = chartData.reduce((iMin, d, i) => d.lstm_hybrid < chartData[iMin].lstm_hybrid ? i : iMin, 0);
    const peakPoint = chartData[peakIdx];
    const minPoint  = chartData[minIdx];
        ...
        <div className="w-full h-full flex flex-col">
            ...
            <ResponsiveContainer width="100%" height="90%">
                <ComposedChart data={chartData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
                    ...
                    {/* Fix 14: Peak annotation */}
                    {peakPoint && (
                        <ReferenceDot
                            x={peakPoint.time} y={peakPoint.lstm_hybrid}
                            r={4} fill="#FF2A6D" stroke="#0B0F1A" strokeWidth={2}
                            label={{ value: `▲ ${peakPoint.lstm_hybrid.toLocaleString()} MW`, position: 'top', fill: '#FF2A6D', fontSize: 10, fontFamily: 'monospace' }}
                        />
                    )}

                    {/* Fix 14: Minimum annotation */}
                    {minPoint && (
                        <ReferenceDot
                            x={minPoint.time} y={minPoint.lstm_hybrid}
                            r={4} fill="#00FFF6" stroke="#0B0F1A" strokeWidth={2}
                            label={{ value: `▼ ${minPoint.lstm_hybrid.toLocaleString()} MW`, position: 'bottom', fill: '#00FFF6', fontSize: 10, fontFamily: 'monospace' }}
                        />
                    )}
                    ...
                    {/* Fix 15: Brush uses dataKey="time" with a smart tickFormatter
                        to show MM-DD HH:MM for multi-day forecasts instead of
                        repeating "00:00" labels that the old displayTime key caused. */}
                    <Brush
                        dataKey="time"
                        height={25}
                        stroke="#00FFF6"
